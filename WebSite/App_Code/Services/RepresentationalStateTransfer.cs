...
using RayMueller.Security;
using RayMueller.Web;

namespace RayMueller.Services
{
        ...
        protected virtual void CombineScripts(HttpContext context, bool isSaaS, string scriptName, string culture, string version)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                	...
                	sb.AppendFormat(String.Format("$(\'<link></link>\').appendTo($(\'head\')).attr({{ href: \'{0}/App_Themes/RayMueller/_" +
                                "Theme_Yoshi.css?{0}\', type: \'text/css\', rel: \'stylesheet\' }});", ApplicationServices.Version), baseUrl);
                try
                {
                ...
                sb.AppendFormat("var __targetFramework=\'4.6\';__tf=4.0;__cothost=\'appfactory\';__appInfo=\'WOCTrack|{" +
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            if (!(String.IsNullOrEmpty(callback)))
            	...
            	if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("RayMueller=typeof RayMueller==\'undefined\'?{{}}:RayMueller;RayMueller.{0}=", instance);
                }
        }
